...
| バージョン | 日付 | 内容 |
|-----------|------|------|
| v1.6 | 2026-02-09 | Phase 4完了（検索画面の実装） |
| v1.5 | 2026-02-09 | シーズン管理機能の追加（シーズン追加・編集・削除画面） |
| v1.4 | 2026-02-09 | Phase 3完了（予定登録・編集画面、ホーム画面の試合登録導線） |
| v1.3 | 2026-02-06 | Phase 2完了（シーズン一覧・詳細、HOMEチーム詳細） |
| v1.2 | 2026-02-05 | Phase 1完了（ログイン、ホーム画面の基本実装） |
| v1.1 | 2026-02-05 | データモデル実装完了、画面設計詳細化 |
...
graph TB
    ...
    LeagueList -->|シーズン追加FAB| SeasonForm
    LeagueList -->|シーズン編集ボタン| SeasonForm
    LeagueList -->|チームをタップ| TeamDetail

    SeasonForm -->|保存・削除・キャンセル| LeagueList
...
graph TB
    subgraph "リーグ一覧画面"
        ...
        SeasonList["📋 シーズンリスト<br/>━━━━━━━━━━━━━━━━<br/>▸ 2026シーズン (12試合) [✏️]<br/>▸ 2025シーズン (18試合) [✏️]<br/>▸ 2024シーズン (15試合) [✏️]<br/>━━━━━━━━━━━━━━━━<br/>[+] シーズン追加 FAB"]
...
| **シーズンリスト** | 年度別のシーズンと試合数を表示。**日本代表は除外**（チームタブに表示）。タップで詳細画面へ。各シーズンに編集ボタン[✏️]を配置 |
| **シーズン追加FAB** | 右下のフローティングアクションボタン。タップでシーズン登録画面へ遷移 |
...
```mermaid
graph TB
    subgraph "シーズン登録・編集画面"
        Header8["ヘッダー: 新しいシーズン / シーズンを編集"]
        DeleteButton["[削除ボタン] (編集モードのみ)"]

        Form["📋 フォーム<br/>━━━━━━━━━━━━━━━━<br/>シーズン名: [_______________]<br/>━━━━━━━━━━━━━━━━<br/>年度: [_______________]<br/>━━━━━━━━━━━━━━━━"]

        SaveButton["[追加/更新]<br/>━━━━━━━━━━━━━━━━<br/>[キャンセル]"]
    end

    Header8 --> DeleteButton
    DeleteButton --> Form
    Form --> SaveButton
```
...
| 要素 | 説明 |
|------|------|
| **削除ボタン** | 編集モードのみ表示。タップで確認ダイアログ表示後に削除 |
| **シーズン名** | テキスト入力（例: "2026シーズン", "2026 J1リーグ"） |
| **年度** | 数値入力（例: 2026） |
| **追加/更新ボタン** | 新規登録時は「追加」、編集時は「更新」と表示 |
| **キャンセルボタン** | 変更を破棄して前の画面に戻る |
...
### 試合追加フロー
1. シーズン詳細 / チーム詳細（日本代表のみ）/ ホーム画面から「予定を追加」をタップ
2. 予定登録・編集画面で新規入力
3. 「保存」で元の画面に戻る

### シーズン追加フロー
1. シーズン一覧画面のFABボタン（+）をタップ
2. シーズン登録画面でシーズン名と年度を入力
3. 「追加」で一覧画面に戻る

### シーズン編集・削除フロー
1. シーズン一覧画面で各シーズンの「編集」ボタン（✏️）をタップ
2. シーズン編集画面で内容を変更、または「削除」ボタンをタップ
3. 「更新」または「削除」確認後、一覧画面に戻る
...
### v1.6 (2026-02-09)
- Phase 4の実装完了：検索画面
- 検索画面（SearchScreen）の実装
  - キーワード検索機能（チーム名、メモ、スコアで検索）
  - シーズン、HOMEチーム、対戦相手、勝敗でのフィルタリング
  - 検索結果の詳細表示（得点者、メモを常に表示）
  - 検索結果からシーズン詳細画面への遷移
- ホーム画面とシーズン一覧画面に検索画面への導線を実装

### v1.5 (2026-02-09)
- シーズン管理機能の追加
- シーズン登録・編集画面（SeasonFormScreen）の実装
  - 新規登録モード：シーズン名と年度を入力して新規作成
  - 編集モード：既存シーズンの編集
  - 削除機能：確認ダイアログ付き
- シーズン一覧画面にシーズン追加FABと編集ボタンを追加
- 画面設計書の画面遷移図とフロー説明を更新
